...
Given(`que soy un usuario de GDE tipificado con referencia {string} {string} {string}`, (cp: string, user: string, pass: string) => {
    // [Given] Sets up the initial state of the system.
    console.log(cp)
    login.cambioUsuarioYcontrasena(user, pass);
});

When(`en la api cambio de valor de GDE, envio una referencia perteneciente a una guia con nivel de servicio veintidos estado diferente a recibir por coordinadora, antes de autorizar a carga de trabajo, con el campo valor igual a cero {string} {int} {int}`, async (referencia: string, valor: number, autorizada: number) => {
