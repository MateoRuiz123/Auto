...
Given(`Agregar con referenciaDetalle que soy un usuario de GDE tipificado con referencia y referencia_detalle {string} {string} {string}`, (cp: string, user: string, pass: string) => {
    // [Given] Sets up the initial state of the system.
    console.log(cp)
    login.cambioUsuarioYcontrasena(user, pass);
});

When(`Agregar con referenciaDetalle en la api cambio de valor de GDE, envio una referencia y referencia_detalle pertenecientes a una guia con nivel de servicio uno estado diferente a recibir por coordinadora, antes de autorizar a carga de trabajo, con el campo valor mayor que cero {string} [{string}] {int} {int}`, (referencia: string, referenciaDetalle, valor: number, autoirzada: number) => {
