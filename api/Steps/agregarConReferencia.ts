...
Given(`Agregar con referencia que soy un usuario de GDE tipificado con referencia {string} {string} {string}`, (cp: string, user: string, pass: string) => {
    // [Given] Sets up the initial state of the system.
    console.log(cp)
    login.cambioUsuarioYcontrasena(user, pass);
});

When(`Agregar con referencia en la api cambio de valor de GDE, envio una referencia perteneciente a una guia con nivel de servicio uno estado diferente a recibir por coordinadora, antes de autorizar a carga de trabajo, con el campo valor mayor que cero {string} {int} {int}`, (referencia: string, valor: number, autorizada: number) => {
