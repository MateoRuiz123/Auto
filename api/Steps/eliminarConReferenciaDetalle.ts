...
When(`en la api cambio de valor de GDE, envio una referencia y referencia_detalle pertenecientes a una guia con nivel de servicio veintidos estado diferente a recibir por coordinadora, antes de autorizar a carga de trabajo, con el campo valor igual a cero {string} [{string}] {int} {int}`, async (referencia: string, referencia_detalle: any, valor: number, autorizada: number) => {
